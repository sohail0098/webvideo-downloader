...
# xml tool, auto complete namespace
class XMLUtils:
    @classmethod
...
def request(*args, **kargs):
    ...
    if response.status_code > 299:
        reqLogger and reqLogger.error('%s %s http response: %d error' \
            % (method, url, response.status_code))
        raise Exception('%s %s http response: %d error' \
            % (method, url, response.status_code))
    return response
...
def generateFileNames(urls, baseFileName):
    ...
    if len(urls) == 1:
        ...
        for i in range(len(urls)):
            fileName = '%s_First%03dSegment%s' % (baseFileName, i+1, suffix)
...
def checkFFmpeg():
    with os.popen('ffmpeg -v quiet 2>&1') as f:
        if f.read().strip():
            print('Warning!!: ffmpeg command could not be found, merging video might be bugged，' + \
                'Please configure PATH，Or put it in the current directory（Windows only)')

def mergePartialVideos(fileNames, fileName, correct = False):
    print('Merging Files')

    if correct:
        # Fast binary merge, used to correct hls video timestamp
        mergeFiles(fileNames, fileName)
    else:
        # Use concat demuxer to merge => needs same encoding params => does not need same format => greater adaptation
