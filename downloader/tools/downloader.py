...
class WebDownloader:
                ...
                try:
                    ...
                    if response and response.status_code != 206:
                        ...
                        if not self.hasWarned and self.failedSize > self.currSize * 0.5:
                            self.hasWarned = True
                            print('\nWarning!!: The current download failure rate is high, and the server does not support resumable downloads，' \
                                + 'Try to reduce the number of download threads to increase the download speed. command line parameters: -t:h N')
    ...
    def multiThreadDownload(self, url, fileName, headers, threadCnt, fragmentCnt):
        ...
        print('Downloading', fileName)
        print('In %d segment, Parallel %d thread download' % (fragmentCnt, threadCnt))
        self._waitUtilFinish()
    ...
    def multiThreadDownloadAll(self, urls, fileNames, headers, threadCnt, fragmentCnt):
        ...
        print('In %dx%d segment, Parallel %d thread download' % (len(urls), fragmentCnt, threadCnt))
        self._waitUtilFinish()
